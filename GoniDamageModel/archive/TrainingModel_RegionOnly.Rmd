...
raw_data <- read.csv(file="data//All.csv", header=TRUE, sep=",", row.names = 1)
summary(raw_data)
...
phl_admin <- st_read("data/PH_municipality/PH_municipality.shp")
bicol_pcodes <- phl_admin$Mun_Code[phl_admin$Reg_Name == "REGION V (BICOL REGION)"]
length(bicol_pcodes) # 114 municipalities.

```

```{r}
# raw_data <- raw_data[raw_data$pcode %in% bicol_pcodes, ]
raw_data <- raw_data[raw_data$Region == 'central', ]
nrow(raw_data) # 188 datapoints.
```

```{r}
table(raw_data$disaster_name) # 7 typhoons.
