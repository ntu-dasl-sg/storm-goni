...
tdnum.list = replicate(length(ph_mun$Mun_Code),0)
pcprop.list = replicate(length(ph_mun$Mun_Code),0)
pcnum.list = replicate(length(ph_mun$Mun_Code),0)
tdprop.se.list = replicate(length(ph_mun$Mun_Code),0)
tdnum.se.list = replicate(length(ph_mun$Mun_Code),0)
pcprop.se.list = replicate(length(ph_mun$Mun_Code),0)
pcnum.se.list = replicate(length(ph_mun$Mun_Code),0)
windspeed.list = replicate(length(ph_mun$Mun_Code),0)
dist_first_impact.list = replicate(length(ph_mun$Mun_Code),0)
...
ggplot() + theme(
  ...
  scale_fill_gradient(low="white", high="red") + 
  geom_sf(data = goni_trace, size = 0.2, color = "red", fill = "cyan1") + 
  coord_sf(xlim=c(118,127), ylim=c(11,16), expand=F) + labs(fill = "Damage rate \n standard errors", x = "", y = "") + geom_text(data = cap.df, aes(x = x, y = y, label = text), size = 2.5, hjust = 1) +
  theme(legend.position =  c(0.025, 0.15), legend.justification = c(0, 0), legend.title = element_text(size = 7.5), legend.text = element_text(size = 7.5))
```
