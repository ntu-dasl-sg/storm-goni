...
* **Damage metrics**: The damage resulted from the typhoons is measured as the number of buildings that are partially or completely damaged (PD or CD resepectively) per municipality. These metrics are also given as percentage of the total number of buildings per municipality (pPD and pCD respectively). Other columns correspond to different metrics computed from these two basic measurements.
...
* **Site and Exposure metrics**: Variables related to site topographic characteristics and municipality socio-economic indicators. These only depend on the municipality and not on the typhoon (assuming they are relatively constant in time).
  + Slope [-]: Average terrain slope in municipality.
...
For the proportion of buildings damaged:
...
For the proportion of buildings completely damaged:
...
The dataset for the 1st damage model (Proportion of damage buildings):
...
The dataset for the 2nd damage model (Proportion of completely damage buildings):
...
for (i in 1:length(typhoons)){
 ...
 # If the estimate of buildings damaged is less than the estimate of buildings completely damaged, set it to the latter.
...
The total damage model has a significantly higher cross-validation error. In both cases, a large portion of the error seems to be driven by a couple of events that well above the mean error.
...
#### Computed predictions for the 1st model: Total proportion of damaged buildings
...
```{r}
# Predict proportion of all damaged buildings and standard errors
pred.td_prop_bldgs = predict(td_step, newdata = new_td_data, type = "response", se.fit = TRUE)
# Predict number of all damaged buildings
...
#### Computed predictions for the 2nd model: Proportion of completely damaged buildings
...
```{r}
# Predict proportion of all damaged buildings
...
ggsave("graphics/goni-damagerate.png", dpi = 300, height = 6, width = 7.5, units = "in")
```
...
ggplot() + theme(
  ...
  scale_fill_gradient(low="white", high="red") + 
  geom_sf(data = goni_trace, size = 0.2, color = "red", fill = "cyan1") + 
  coord_sf(xlim=c(118,127), ylim=c(11,16), expand=F) + labs(fill = "# Damaged buildings", x = "", y = "") + geom_text(data = cap.df, aes(x = x, y = y, label = text), size = 2.5, hjust = 1) +
  theme(legend.position =  c(0.025, 0.15), legend.justification = c(0, 0), legend.title = element_text(size = 7.5), legend.text = element_text(size = 7.5))

ggsave("graphics/goni-damagedbuildings.png", dpi = 300, height = 6, width = 10, units = "in")
```
...
The top 10 municipalities hit, in terms of number of damaged buildings, are:
...
colnames(spdf@data) <- c("Municipality", "Damage_rate", "Damage_rate_standard_error", "No_of_damaged_buildings", "No_of_damaged_buildings_standard_error", "Complete_damage_rate", "Complete_damage_rate_standard error", "No_of_completely_damaged buildings", "No_of_completely_damaged_buildings_standard_error", "Wind_speed", "Distance_to_first_impact", "Slope", "Elevation", "Population_density", "Proportion_of_strong_roof_type", "Experience")
